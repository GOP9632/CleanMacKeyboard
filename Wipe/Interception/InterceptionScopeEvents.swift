...
extension InterceptionScope {
    ...
    /// 「滑鼠移到這裡了」，選單與按鈕會跟著亮起來。
    ///
    /// **游標本身還是會動，而這是接受的行為。**#12 在真機上驗過：這一層攔得下
    /// 事件，沒有 app 收得到它們，點擊與手勢完全沒有反應，但螢幕上那個箭頭照樣
    /// 跟著手指跑。游標的位置是視窗伺服器直接從 HID 那一層畫的，不經過這個
    /// 攔截點。
    ///
    /// 沒有為了那個箭頭再往下追。兩個候選機制（把攔截點換到 HID 那一層、
    /// 用 `CGAssociateMouseAndMouseCursorPosition` 把游標鎖住）都是全域性的，
    /// 而本 app 有一條不能碰的線：process 一旦結束，鎖必定消失
    /// （見 `CONTEXT.md` 的不變條件）。游標移到哪裡都按不動任何東西，防護的
    /// 實質沒有少，不值得拿那條線去換。畫面上的說明因此不承諾游標會停
